...
**Chat.** `POST /api/sessions/:id/pi-chat` creates a detached PR worktree and a daemon-owned `pi --mode rpc` child. The model is per chat (repo + PR number): stored in `pi-chat-models.json`, shown after resume/refresh, and applied with `--model` / `--thinking` (or a respawn) so Pi's `defaultModel` is untouched. An explicit Pi default is stored as null and, on resume, passed as Pi's own default flags so the session actually switches. Missing entries fall back to a legacy sidecar or the transcript. Chat is per pull request, not per revision SHA: a new push reuses the same Pi transcript and refreshes the worktree to the selected head. `GET` pages projected turns from the Pi JSONL; SSE `pi-chat` events carry the live overlay. The browser never reads the session file. Retention: 14 days; cleanup at daemon start and every six hours; skip the live RPC child and dirty worktrees.
